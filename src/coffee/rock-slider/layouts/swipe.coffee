  ...
  $.fn.rockSlider.swipeLayout = ->
    ...
    @init = ->
      ...
      $('html > head').append $ "<style data-rock-style-id='#{@id}'>" +
        ...
        "transition: transform #{@settings.layout_settings.speed}ms #{easing};" +
        "-o-transition: -o-transform #{@settings.layout_settings.speed}ms #{easing};" +
        "-moz-transition: -moz-transform #{@settings.layout_settings.speed}ms #{easing};" +
        "-webkit-transition: -webkit-transform #{@settings.layout_settings.speed}ms #{easing};" +
        " }" +
        "</style>"

      # Set first setup to determine if we have a resize operation
      @first_setup = true
    ...
    @setup = ->
      ...
      @element.height @parent_width / @settings.width * @settings.height
